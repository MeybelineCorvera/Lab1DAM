...
class Calculadora {
    ...
    constructor(valorPrevioTextElement, valorActualTextElement) {
        ...
        this.historialOperacion = ''; //PARA GUARDAR LA OPERACION COMPlETA Y SE MUESTRE EN EL DISPLAY
    ...
    agregarNumero(numero) {
        ...
        if (this.valorActual.toString().length >= 9) return;
...
porcentajeButton.addEventListener('click', () => {
    // Si no hay valor actual, no hace nada
    if (calculator.valorActual === '') return;

    // Si no hay valor previo, tomamos el valor actual como previo y operación '%'
    if (calculator.valorPrevio === '') {
        calculator.valorPrevio = calculator.valorActual;
        calculator.operacion = '%';
        calculator.valorActual = '';
    } else if (calculator.valorActual !== '') {
        // Si ya hay valor previo y actual, calculamos primero
        calculator.calcular();
        calculator.operacion = '%';
        calculator.valorPrevio = calculator.valorActual;
        calculator.valorActual = '';
